...
namespace LivelyChatBubbles
{
	public class ChatMouthpiece : MonoBehaviour
	{
		...
		public void Speak( string message)
		{
			...
			if (TheGame._instance.mainCanvas && ChatAnchor && ChatOutputProfile && ChatBubblePrefab && chatBubble == null)
			{
				isSpeaking = true;
				chatBubble = GameObject.Instantiate(ChatBubblePrefab, new Vector3(-100000, -10000, 0), Quaternion.identity, TheGame._instance.mainCanvas.transform);
		...
		IEnumerator WaitForFinished()
		{
			...
			chatBubble = null;
			// if(chatType != GameType.none)
			// {
			// 	UIHandler.instance.SetPopUpMessage(chatProcesser.Value, chatType);
			// }
			chatProcesser = null;
			isSpeaking = false;
		}
