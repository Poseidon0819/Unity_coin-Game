...
function StartMic()
{
	unityInstance.SendMessage("WebMic", "NotifyRecordingChange", MicState.Booting);
	...
	if (navigator.getUserMedia)
	{
		navigator.getUserMedia(
			...
			function (e)
			{
				unityInstance.SendMessage("WebMic", "NotifyRecordingChange", MicState.NotActive);
...
function start_microphone(stream)
{
	...
	recorder.onaudioprocess = function (e) 
	{
		...
		unityInstance.SendMessage("WebMic", "LogWrittenBuffer", floatPCM.length);
	...
	unityInstance.SendMessage("WebMic", "NotifyRecordingChange", MicState.Recording);
}
